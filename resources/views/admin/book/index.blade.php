                                            ...
                                            <a href="{{ route('admin.order.create', $order->order_code) }}" class="font-weight-bold">
                                                ...
                                                @if($order->status=='pending')
                                                    ...
                                                    @if($order->OrderItems->count() && $order->order_total != null)
                                                        ...
                                                        <a href="{{ route('admin.book.send-admin', $order->order_code) }}" class="font-weight-bold" onclick="return confirmDelete();">
                                                            Admin |
                                                        </a>
                                                        ...
                                                        @endif
                                                         <a href="{{ route('admin.order.half', $order->id) }}" class="font-weight-bold" onclick="return confirmDelete();">
                                                            Half |
                                                        </a>
                                                        
                                                        
                                                        <a href="{{ route('admin.order.full', $order->id) }}" class="font-weight-bold" onclick="return confirmDelete();">
                                                            Full |
                                                        </a>
                                                        <a href="{{ route('admin.order.tabby', $order->id) }}" class="font-weight-bold" onclick="return confirmDelete();">
                                                            Tabby |
                                                        </a>
                                                ...
                                                {{ $order->status=='discount' ? 'Discount' : '' }}
                                                {{ $order->status=='payment' ? 'Payment' : '' }}
                                                {{ $order->status=='production' ? 'Production' : '' }}
                                                {{ $order->status=='production-processing' ? 'Production Processing' : '' }}
                                                {{ $order->status=='installation-time' ? 'Installation Shedule' : '' }}
                                                {{ $order->status=='installation-processing' ? 'Installation Processing' : '' }}
                                                {{ $order->status=='feedback' ? 'Feddback' : '' }}
                                                {{ $order->status=='complete' ? 'Completed' : '' }}
                                                {{ $order->production_status=='production-completed' ? 'Proudction Completed' : '' }}
                                                {{ $order->installation_status=='installation-completed' ? 'Installation Completed' : '' }}
                                                </span>
