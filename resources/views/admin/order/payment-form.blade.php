...
<head>
    ...
    <title>Checkout | Blinds & Curtains Solution Fzc</title>
...
<body>
    ...
    <nav class="navbar navbar-light bg-white shadow">
        ...
        <div>
            <p class="mb-0 fs-5">Order <b>#{{ $order->order_code }}</b></p>
        </div>
    </nav>
    ...
    <div class="col-lg-5 mb-3">
        <div class="card summary">
            ...
            {{--<p class="d-flex justify-content-between mb-0">
                <span>Price After {{ $order->order_coupon ? $order->order_coupon . '%' : '' }} Discount</span>
                <span style="font-weight:bold;">AED {{ number_format($discountAmount, 2) }}</span>
            </p>--}}
            
            <p class="d-flex justify-content-between mb-0">
                <span>Discount :</span>
                <span style="font-weight:bold;">{{ $order->order_coupon ? number_format($order->order_coupon,2) . '%' : '0.00 %' }}</span>
            </p>
            ...
            <p class="d-flex justify-content-between mb-0">
                <span>Coupon :</span>
                <span style="font-weight:bold;">{{ number_format($order->coupon, 2) }}%</span>
            </p>
            @endif
            ...
            @elseif ($order->payment_status == 'full')
            @elseif (is_null($order->payment_status))
            <p class="d-flex justify-content-between mb-0 text-danger">
                <span>Due Amount :</span>
                <span style="font-weight:bold;">AED {{ number_format($total - $paid, 2) }}</span>
            </p>
            ...
            <form action="{{ route('admin.order.pay') }}" method="POST" class="mt-3" id="paymentForm">
                @csrf
                @if ($order->payment_status == 'half')
                    <select class="form-control mb-4" id="payment_status" name="payment_status" required>
                        <option disabled selected>Select Payment</option>
                        <option value="full">Full Payment</option>
                    </select>
                @elseif ($order->payment_status == 'full')
                    <option disabled selected>Full Paid</option>
                @elseif (is_null($order->payment_status))
                    <select class="form-control mb-4 form-select" id="payment_status" name="payment_status" required>
                        <option disabled selected>Select Payment</option>
                        <option value="full">Full Payment</option>
    ...
    {{--<script>
        $(document).ready(function() {
            ...
            var totalPrice = {{$discountedTotal}};
    ...
    <script>
        $(document).ready(function () {
            $(".order-status").on("change", function () {
                ...
                var status = $(this).is(":checked") ? 1 : 0; // Get checkbox status
        
                $.ajax({
                    url: "{{ route('orderitems.updateStatus') }}", // Update with your route
                    type: "POST",
                    data: {
